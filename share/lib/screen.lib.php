...
function topMenu($config)
	...
	if (!empty($topMenuHTML)) {
		$topMenuHTML="
		<div id='contentText' class='col-sm-12'>
		<!-- div id='speedMenu' class='col-sm-2'>
		<button title='Gyógyszer' class='buttonBox' name='modulOperation' type='submit' value='start::medicine::0'><i class='fa fa-medkit' aria-hidden='true'></i></button>
		<button title='Vérnyomás' class='buttonBox' name='modulOperation' type='submit' value='start::bloodpressure::0'><i class='fa fa-heart' aria-hidden='true'></i></button>
		<button title='Edzés' class='buttonBox' name='modulOperation' type='submit' value='start::training::0'><i class='fa fa-user-plus' aria-hidden='true'></i></button>
		<button title='Videó' class='buttonBox' name='modulOperation' type='submit' value='start::video::0'><i class='fa fa-film' aria-hidden='true'></i></button>
		<button title='Recept' class='buttonBox' name='modulOperation' type='submit' value='start::recipe::0'><i class='fa fa-list' aria-hidden='true'></i></button>
		</div -->
		<!-- div class='col-sm-10' --><div id='topMenuText'><div id='topMainMenu'><div id='menuBox'>".$topMenuHTML;
		$topMenuHTML .= "</div><div style='clear:both'></div><!-- /div -->";
		if ($topSubMenuAdd==1)
			{
			$topMenuHTML .= $topSubMenuHTML;
			}
		$topMenuHTML .= "</div></div></div>";
	}
	return $topMenuHTML;
	}
...
function loginService($config)
	...
	if ($_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserLevel"] != 1) {
		if (setConfigValue($config,"configShare","onlyView") != 1 ) {
			...
			$loginServiceHTML .= "<button title='".$config["configShare"]["passwordChangeTitle"]."' id='buttonTop' name='modulOperation' type='submit' value='item::passchange::core_users::".@$_SESSION[$config["configShare"]["whichSystemAndMode"]."status_UserID"]."::".$config["modulSelect"]."'><i class='fa fa-key' aria-hidden='true'></i></button>";
			if ($_SESSION[$config["configShare"]["whichSystemAndMode"] . "status_UserLevel"] != 1)
				{
		...
		$loginServiceHTML .= 
			'<div id="loginBox" style="padding-top:1px;">
					<form id="loginForm" method="post" action="">
						...
						<div id="itemView">
							<input class="form-control" type="text" name="x_nickname" id="x_nickname" value="" />
						</div>
						<div id="itemTitle">
						...
						<div id="itemView">
							<input class="form-control" type="password" name="x_password" id="x_password" value="" />
						</div>
...
//		$config["sharedNavigationAdd"] .= "<button id='descriptionView' title='".$config["configShare"]["descriptionTitle"]."' class='buttonBox' name='modulOperation' type='submit' value='description::description::".$config["modulSelect"]."::'><span data-caption='Description' class='glyphicon glyphicon-comment ewIcon' data-phrase='DescriptionLink'></span></button>";
		...
		// list count select	
		$config["sharedNavigationAddTop"]=$config["sharedNavigationAdd"];
		$listCountOptions =  array( 5,10,20,50);
		$listCountType = '<select id="listCountTop" name="listCountTop" class="form-control" style="width:80px;" value="start::'.$config["modulSelect"].'::'.($config[$config["modulSelect"]]["startRec"]).'" onchange="this.form.submit();">';
		for( $i=0; $i<count($listCountOptions); $i++ ) {
//			if ($listCountOptions[$i] == $_REQUEST["listCountTop"]) {
			if ($listCountOptions[$i] == $config["configShare"]["displayRecs"]) {
		...
		$config["listCountSelectTop"].= '<div id="listCountTop" style="right:center;width:80px;float:left;margin-left:10px">'.$listCountType.'</div>';
		$config["sharedNavigationAddTop"] .=$config["listCountSelectTop"];
		
		
		$config["sharedNavigationAddBottom"]=$config["sharedNavigationAdd"];
		$listCountOptions =  array( 5,10,20,50);
		$listCountType = '<select id="listCountBottom" name="listCountBottom" class="form-control" style="width:80px;" value="start::'.$config["modulSelect"].'::'.($config[$config["modulSelect"]]["startRec"]).'" onchange="this.form.submit();">';
		for( $i=0; $i<count($listCountOptions); $i++ ) {
//			if ($listCountOptions[$i] == $_REQUEST["listCountBottom"]) {
			if ($listCountOptions[$i] == $config["configShare"]["displayRecs"]) {
				$listCountType .= '<option value="'.$listCountOptions[$i].'" selected="selected">' . $listCountOptions[$i] . '</option>';
			} else { 
				$listCountType .= '<option value="'.$listCountOptions[$i].'">'.$listCountOptions[$i] . '</option>';
		...
		$listCountType .= '</select>';
		$config["listCountSelectBottom"].= '<div id="listCountBottom" style="right:center;width:80px;float:left;margin-left:10px">'.$listCountType.'</div>';
		$config["sharedNavigationAddBottom"] .=$config["listCountSelectBottom"];
	...
	if (setConfigValue($config,$config["modulSelect"],"noNavigationDefault")==0)
		...
		$config["navigationAddTopHTML"]=$config["sharedNavigationAddTop"];
		$config["navigationAddBottomHTML"]=$config["sharedNavigationAddBottom"];
